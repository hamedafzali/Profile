...
const NotFoundPage = ({ match }) => {
  ...
  return (
    <div>
      <h1>Whoops!</h1>
      <p>
        <strong>{url.replace("/", "")}</strong> could not be located.
      </p>
    </div>
  );
};
...
const Header = () => {
  ...
  const checkActive = (match, location) => {
    ...
    return (
      pathname.indexOf("/tophergates") !== -1 ||
      pathname.indexOf("/_display/") !== -1
    );
  };
    ...
    <header>
      ...
      <nav>
        <ul className="navLinks">
          ...
          <li>
            <NavLink to="/tophergates" isActive={checkActive}>
              Home
            </NavLink>
          </li>
          <li>
          ...
          <li>
            <NavLink to="/random-page">404 Page</NavLink>
          </li>
        </ul>
      </nav>
    </header>
...
const Footer = () => {
  return (
    <footer>
      <p>
        Made with <span className="heart">&lt;3</span> by tophergates
      </p>
    </footer>
  );
};
